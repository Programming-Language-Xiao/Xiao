...
fn abi_field_type(ty: &IrType) -> Option<u32> {
    match ty {
...
fn name_key(name: &IrName) -> String {
    format!(
        "{}:{}",
...
fn scope_is_ancestor(scopes: &[xiao_ir::IrScope], ancestor: u32, descendant: u32) -> bool {
...
/// 递归判断语句是否实际触及容器值。
fn statement_uses_container_abi(statement: &IrStatement) -> bool {
    match &statement.kind {
        IrStatementKind::Expression { value }
        | IrStatementKind::Assignment { value, .. }
