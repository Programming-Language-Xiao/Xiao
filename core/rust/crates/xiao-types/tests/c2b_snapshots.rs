...
fn assert_snapshot(raw: &str) {
    let snapshot: SnapshotFile = serde_json::from_str(raw).expect("C2-B 快照 JSON 必须有效");
    assert_eq!(snapshot.stage, "03D");
...
#[test]
/// 合法快照覆盖推导并集、显式完整并集、`none` 和严格类型隔离。
fn valid_snapshot_is_stable() {
    assert_snapshot(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../../../tests/spec/05-containers/c2b-valid.json"
    )));
}

#[test]
/// 错误快照覆盖显式成员冲突、成员判断冲突和非集合初始化器。
fn error_snapshot_is_stable() {
    assert_snapshot(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../../../tests/spec/05-containers/c2b-errors.json"
    )));
}
