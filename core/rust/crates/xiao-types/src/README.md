...
有序选择器计划、C2-A 集合静态检查和 AST 类型检查实现。对应工程期 02–04、03-C0、
03-C1、03-C2A、08；不得依赖
...
集合职责：`set_types.rs` 保存 `SetType`、C2-A 可哈希判定和集合赋值兼容规则；
`set_checker.rs` 负责集合字面量、`set()`、成员判断及相关 Runtime 检查标记。
本阶段不创建 Runtime 集合、不执行哈希或集合代数；异构元素和元组递归哈希留给后续 C2。
