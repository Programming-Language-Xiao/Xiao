...
`xiao.lock.json`，并支持既有环境映射原子更新。图节点独立于
...
稳定诊断与跨平台原子文件替换，`src/diagnostics.rs` 放置稳定编号。
远程源适配器、联邦索引、版本求解和 CLI 命令属于后续批次；命令参数和提示符放在 CLI。

## 禁止事项

不执行包安装脚本或包代码，不绘制提示符，不把不同源的版本简单按高低混选；E2A 不实现
远程下载、缓存清理、`sync`、`install`/`i` 或 CLI 命令。
