...
这里承载已迁入生产路径的 09R2 编码器内部实现。父级 `encode.rs` 是生产公开门面，保留公开
类型、错误和编码/解码入口；本目录按职责拆开字节读写、标签映射、编码、解码、输入
校验和回归测试，避免单个 Rust 文件承担整个编码协议。

## 工程期

对应工程期 09（沿革为 09R2）。冻结编码只在内存中的 `Vec<u8>` 上工作，不生成或承诺正式的
...
- `tests.rs`：布局版本 3、41 个 opcode、两种操作数宽度、源码映射和损坏输入的往返/拒绝
  测试，并用显式数值钉住集合算子标签；布局版本 1/2 会在读取函数体前被拒绝。

09R2H 在随机种子计划之后、`unsupported` 之前追加表定义段：每项写入静态签名的
JSON 表示、字段初始化函数索引和成员键到方法索引的映射。函数索引使用所选操作数宽度，
签名复用 IR 镜像；解码拒绝重复方法键，输入校验拒绝重复表、非法签名和悬空引用。

子模块只通过 `pub(super)` 暴露给门面和同级实现，外部调用方应使用父级 `encode` 的公开
入口；`research::encode` 仅是兼容别名。
...
- 门面只负责装配、调用和公开 API；生产模块不能依赖 `tests`，测试模块也不能成为
  任何生产路径的依赖。
- 禁止循环依赖、反向依赖和从目录外绕过 `encode` 访问这些私有模块。源码级
