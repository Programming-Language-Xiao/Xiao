...
实现 Token、缩进/反缩进、注释、表达式、代码块、声明和 AST。数组路径、选择器、表头和反引号标识符在这里保留源码位置；当前已完成 01/L0/L1/L2 词法器、严格最小 P0 AST/解析器、P1 表达式/选择器首批解析以及 P2 标量/const 声明语法。

## 工程期

01–04；当前已交付 F0/L0/L1/L2/P0/P1 表达式与选择器首批及 P2 声明 AST；类型检查与容器语义仍由 02/03 负责，08 负责把 AST 交给名称解析和类型化 IR。
...
P2 声明回归位于 `tests/p2_declarations.rs` 和 `tests/p2_snapshots.rs`；声明节点只保存
标量类型和初始化表达式，不执行类型检查。
