...
按职责放置 Token、词法器、P0/P1/P2 AST、Pratt 解析器、诊断编号和选择器类型。对应工程期
01–04；P1 首批和 P2 标量声明语法已完成，P2-A 已将实现拆为 `token.rs`、`lexer.rs`、`ast.rs`、
...
已加入表达式核心、调用、转换、索引路径、选择器和标量/const 声明 AST，但不执行容器或类型语义。所有公共
