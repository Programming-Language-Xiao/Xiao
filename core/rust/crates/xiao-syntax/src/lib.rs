//! Xiao 词法 Token、语法树与前端解析器的基础实现。
//!
//! 本模块负责把经过 UTF-8 校验的源码转换为带 [`SourceSpan`] 的 Token
//! 流。当前覆盖 01 阶段的 L0、L1、L2、严格最小 P0 与 P1：源码位置、基本
//! 字面量、保留字、括号、基础运算符、反引号名称、文档注释、缩进 Token，以及
//! 从字面量/名称到表达式、调用、转换、赋值和索引选择器的 AST/解析器入口。
//! 词法器和解析器不会执行 Xiao 程序、推断类型或进行容器边界检查。
...
/// P1 选择器路径和选择项的 AST 数据结构实现。
mod selectors;

/// 重新导出选择器相关的公开 AST 类型，供前端和后续语义阶段复用。
