...
不适用于字典表或集合。类型阶段验证数量、来源类型和边界并记录随机计划；09R2 研究 VM
已按该计划执行真实抽样。正式 `xiao run` 和 `.xiaoc` 产物仍未开放。
...
动态数量的实际合法性和候选集长度由研究 VM 在执行时验证；类型阶段不会为了生成一个
静态空容器而猜测随机结果。
...
类型阶段不会推进随机状态。研究 VM 通过 `RandomSource` 注入确定性随机源；`random.seed`
会更新当前执行上下文，固定种子可复现抽样序列。没有显式设置种子时，研究 VM 使用默认
确定性源；这不构成生产随机源 API 的承诺。

运行时数量错误使用 `X06-RUNTIME-019`，种子错误使用 `X06-RUNTIME-020`；两者都是可恢复
错误，会沿普通 handler/finally 路由传播。
