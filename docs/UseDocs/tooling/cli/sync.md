...
在含 `config.xiao` 的项目内运行 `xiao sync`。它向上发现项目根，解析本地路径依赖及
`[sources]` 中已声明的远程版本依赖，
...
远程包仅在首次锁定、显式更新或锁定缓存缺失时读取完整索引/正文；已锁定的 Git 标签
即使正文缓存命中，也须在线复核标签指向的提交及快照清单；不可达时明确失败。
正文须通过锁定摘要、经安全 TAR 展开，且 `config.xiao` 的包名和版本与索引一致，
才可进入环境映射。普通 `sync` 不隐式升级远程版本，
`install` 也不重新求解。此闭环不执行安装脚本、不提供预编译 `.xiaoc` 变体；
独立 `{ git = "..." }` 依赖仍会明确拒绝，不等同于索引源内的 Git 包。
