...
**别名层清理已具名为 `09-B0-F`「研究别名层清理」**（登记见
[09-B0](09b0-bytecode-closure.md) §2.1），由后续批次做。
...
### 8.3 09R3 冻结第 4 项逐条对照

本批新增的是一条终止通道，并新调用既有转移机制；它没有改写冻结后的 ABI 或既有
`Error`/`Fatal` 语义。按 `09r3:92` 的三条口径逐项核对如下：

| 冻结第 4 项口径（`09r3:92`） | 本批是否改动 |
| --- | --- |
| handler 路由 | ❌ 未改——`Cancelled` 走 `route_fault_scoped` 的 `else` 分支，根本不进入 handler 查表逻辑。 |
| `finally` 子程序 | ❌ 未改——`run_cancelled_finalies` 是对既有 `finally` 机制的新调用，子程序自身的执行语义没有变化。 |
| 释放序列的转移语义 | ❌ 未改——`Cancelled` 复用 `unmatched_error` 计划，计划本身与 `Error` 走的是同一份。 |

因此本批不触发 `09r3:27` 的作废规则。

---

## 九、本批收尾结果（2026-09-24）

本节列出的四项收尾已经全部完成。核心设计与 §二/§三 逐条吻合：三条通道语义正确、
两处循环对称接入、计数器跨帧单调、退出码保持 `2`，性能对照仍独立于 09R3 冻结数字。

### 9.1 `Fault` 注释已与三条通道同步

`core/rust/crates/xiao-vm/src/semantics/exec.rs` 的 `Fault` 文档现在明确区分：

- `Error` 参与 handler 路由；
- `Cancelled` 不查 handler 表、不进入用户 `catch`，但执行 `finally` 与既有释放计划；
- `Fatal` 不执行释放计划，也不能被普通处理器捕获。

这与 `unwind` 的 doc 注释使用同一套分类，不再保留“只有两条通道”的旧表述。

### 9.2 09R3 冻结第 4 项已完成逐条对照

上面的 §8.3 已按 `09r3:92` 给出 handler 路由、`finally` 子程序和释放序列转移语义
三条口径的逐条结论，并明确这是新增终止通道而不是 ABI 改动，因此不触发作废规则。

### 9.3 提交正文门禁已从提醒升级为固定机制

`tools/repo-check` 新增 `commit-msg` 检查命令，固定拒绝无规范前缀的标题和空正文；
`.githooks/commit-msg` 在提交前调用该检查器。首次在本地启用钩子：

```text
cmd /c git config core.hooksPath .githooks
```

历史上的六笔空正文提交（`7ec5cb6`、`f5043c2`、`d9a8efc`、`47d1b90`、`b6bed61`、
`8028cac`）保持原样，不改写历史；后续提交由钩子在提交前拦截。

### 9.4 日期已按 Git 实际提交时间校正

Git 记录 `8028cac` 的提交时间为 **2026-09-24**，因此 §八与本节统一采用该日期。
