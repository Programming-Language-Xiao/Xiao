...
检查点记为 `B0-C-CANCEL-001`，出口批次经 X0-E 复核后改为 `09-B0-E`，不会把边界采样
伪装成中途中断能力。
...
| 取消 / 超时 | ✅ 驱动器边界采样已实现；VM 中途检查点记为 `B0-C-CANCEL-001`，出口 `09-B0-E` |
