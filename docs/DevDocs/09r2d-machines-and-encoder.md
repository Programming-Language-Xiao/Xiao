...
`r2_stack.rs` 当前为 **43** 条回归（其中 40 条为交接时的 R2C 基线，另含机型指标、
pc 映射反例，以及栈映射口径的差分测试）；编码器单元测试覆盖 31 个 opcode、
两种操作数宽度和拒绝路径。H1 已用
...
> **状态：R2D 已完成，本结论的前半已兑现。** 按此排序，**下一个要实现的批次是 R2b
> 选择器全量**（它的交接文档见 [09R2B](09r2b-selector-execution.md)）。下面保留当初的
...
- 本批文档门禁已补入实际提交号、27 条共享向量、40 条 R2C 栈式基线（当前栈式回归
  共 43 条，新增三条区分度测试）和 R2D 兼容门；正式全量门禁仍在最终收尾时复跑。
