...
只有本批明确排除的 `for`/表声明、`Result` 泛型与正式 `.xiaoc` 格式；R2b 选择器全量已由
后续批次交付。

**R2D 交付快照（2026-09-18，保留为 R2D 基线）**：实现提交为 `64691b6`、`351cb12`、
`9b7d887`、`c3bd94c`、`2ac2f2e`、`af779e9`、`1a2b22f`、`b1bab53`、`1605282`。当时三种载体
共用 27 条向量，`r2_stack.rs` 为 **43** 条回归；编码器单元测试覆盖 31 个 opcode、
两种操作数宽度和拒绝路径。H1 已用
`subroutine_faults` 栈化状态收敛；H2 去掉块级 `TacInstr` 克隆，交接基准为中位数
`253.66 ms -> 245.83 ms`（约 `3.1%`）。

**R2B 扩展后的当前事实（2026-09-19）**：选择器执行追加 `SelectorApply`、`BroadcastAssign`
和 `RandomSeed` 三个 `TacOp`，稳定 opcode 表现为 **34 个变体**；共享向量增至 **31 条**
（新增 `selectors.json` 4 条），`r2_stack.rs` 当前为 **53** 条测试。R2B 已启用
`selector_bounds`、`selector_step`、`random_count`、`random_seed`，对应 `X06-RUNTIME-017`
至 `X06-RUNTIME-020`；三种载体继续复用同一语义核和向量。
...
### 与 R2b 的排序结论：**R2D 先做（已兑现）**

> **状态：R2D 与 R2B 均已完成。** 按此排序，R2B 已在 R2D 之后交付（交接文档见
> [09R2B](09r2b-selector-execution.md)）。下面保留当初的
> 排序理由，作为「为什么是这个顺序」的单一来源。
...
**已知代价已兑现并关闭**：R2D 基线编码器为 31 个 `TacOp` 变体；R2B 随后追加三个选择器
操作，现行编码器与三种载体已同步到 34 个变体。反过来先做 R2b 会让指令集先定型再写编码器，
但会把 09R3 推后；这是当时的取舍，不是遗漏。
...
这些边界由 `r2_stack.rs` 的 40 条 R2C 基线回归（当前文件共 43 条，新增三条区分度测试）
...
| RuntimeCheck | R2D 基线只执行当时降低器发出的四类检查；R2B 已追加并实际执行 `selector_bounds`、`selector_step`、`random_count`、`random_seed`，集合/迭代器类别仍留在 `TacProgram.unsupported` |
...
**安全网**：三机型跑同一组 `tests/spec/09-bytecode/*.json`（当前 **31 条**：
`containers` 8 / `errors` 9 / `scalar` 6 / `control` 4 / `selectors` 4），再加上
`r2_stack.rs` 的 53 条测试。期望值含完整 `releases` 事件序列；分配器、活析或选择器
语义算错会直接表现为向量失败。
...
**数据面事实（R2B 扩展后）**：`TacOp` 共 **34 个变体**；含 `String` 操作数 **6 处**（`NewDictTable`/
...
这些都是体积杠杆。34 个变体包含 R2C 新增的 `MakeError`、`CallSub`、`RetFromSub` 和带
`value` 操作数的 `Check`，以及 R2B 的 `SelectorApply`、`BroadcastAssign`、`RandomSeed`；
编码器不能按 R2C 之前的旧指令子集实现后再“补几个 opcode”。
...
- 本批文档门禁已补入实际提交号和 R2D 历史基线；R2B 收尾后当前为 31 条共享向量、
  53 条栈式测试（含 4 条选择器向量与动态/随机/字典列回归）。正式全量门禁仍在最终收尾时复跑。
...
- `tests/spec/09-bytecode` 的 31 条向量现在由 `r2_vectors.rs` 的同一观察器依次消费栈式、
  分类型寄存器式和混合式三种载体；JSON 仍不含机型字段。正式性能/内存门槛仍留给 09R3。

---

## 五、提交切分

按可独立验证的单元分八次，**每次提交后门禁全绿**；R2D 施工时先跑 09R2C 的 40 条栈式回归，
确认语义核没有漂移：
...
5. 三机型共用向量对拍（`r2_vectors.rs` 泛型化）+ `VmMetrics` 扩展 + 清 H2 的 clone（附基准数字）；
   此时 27 条 JSON 必须零差异，40 条栈式回归仍全部通过；R2B 收尾后总计为 31/53。
6. 指令编码器 + 往返解码 + 两种操作数宽度变体 + 体积对比；R2D 基线覆盖 31 个 `TacOp` 变体，
...
2. **09R2C 栈式行为零漂移**：Carrier 第一步之后，R2D 基线的 27 条 JSON 向量和 `r2_stack.rs` 的
   40 条回归逐条不变；R2B 收尾后扩展为 31 条向量和 53 条测试；catch/重抛、嵌套 finally、
   覆盖性退出、suppressed、Fatal 和四类 Check
...
7. **编码可往返**：R2D 基线的 31 个、R2B 收尾后的全部 34 个 `TacOp` 变体在 `TAC → bytes → TAC` 中等价（`f64` 按位比较）；
