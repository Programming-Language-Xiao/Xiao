...
- **§8.1 已完成**，由 `5ba9a0d` 修复：门面重导出行补上了文档注释，13 个内部项也补齐，
  `bun run check` 恢复 **exit 0**，公共 API 覆盖率回到 100%。
- **§8.2 已完成**，说明补在 [11X0-G](11x0g-type-checker-decoupling.md) §八的
  「无语义变更的验证依据」一节。**未改写历史**——那 8 个提交仍是空正文，
  说明与证据落在文档里，指向它们的正是本节。
- 时序上 `3271c7b`／`9816481`（G 批收尾）与 `5ba9a0d` 都发生在 `c2f0b2b`（本批第一步）
  之前，**两批的说明没有混在一起**。
...
之后各批守住 → **`checker.rs` 那批 8 个提交正文为空，再次复发**（该批的补正见 §8.2）。
