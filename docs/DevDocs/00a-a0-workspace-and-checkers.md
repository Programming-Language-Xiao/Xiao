...
- 本地入口：`bun run check` 执行全套门禁；需要单项结果时使用 `bun run check:layout`、`bun tools/repo-check/src/cli.ts docs|usedocs`、`bun run check:coverage`、`bun run check:clippy` 或 `bun run check:lock`。超标 Rust 文件会让 `check:layout`/`check` 调用 Rust 适配器，因此需有 Rust 工具链、预先构建适配器或设置 `XIAO_RUST_DOC_ADAPTER`。
...
- **Clippy 门禁（`check:clippy`，2026-09-25 加入）**：`cargo clippy --workspace --all-targets -- -D warnings` 是 `bun run check` 的固定组成部分；CI 另在 Linux amd64 复跑一次，其他平台沿用各自的格式、测试和原生复现步骤。Clippy 不再只是提交前手跑清单。
