...
| [00D. Rust AST 适配器协议 v2 修正交接](00d-doc-adapter-protocol-v2-fixes.md) | 修正 `0e9a42b` 的 `end_line` 死字段、大纲行数口径与协议契约文档，并给请求体加大纲开关；是单文件行数门禁的前置 | 未开始（审核发现：`end_line` 恒等于 `line`） |
...
| 00D | [Rust AST 适配器协议 v2 修正交接](00d-doc-adapter-protocol-v2-fixes.md) | `Declaration.end_line` 真实语义、大纲 `lines` 口径统一、`declaration_head` 文档对齐、`outline` 请求开关、协议 UseDoc 同步 | 未开始（审核发现六条） |
