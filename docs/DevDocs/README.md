...
| 11A-CONC | [并发模型：工具链内部并发与语言级并发的边界](11a-concurrency-model.md) | 划清工具链内部并发（E3B 前置，**冻结**）与语言级并发（归 `07-D`，**不冻结**）；冻结原语选型、有界并行、结果确定性、失败语义、跨进程锁 | 已完成设计冻结（§一 已写进 `00-decisions`；E3B 实现与测试待做，语言级并发未冻结） |
| 11A-E3B | [快速解析与远程缓存](11ae3b-fast-resolution-and-cache.md) | 四类缓存分离（内容寻址 vs 源身份覆盖）、快速路径三类判据、三态源状态、有界并行与跨进程锁；**仍不联网，本地源驱动** | 待开工（交接文档已就绪；**前置 11A-CONC 已冻结**） |
| 11A-E3C | [静态源与 GitHub 适配器](11ae3c-static-and-github-sources.md) | 静态 HTTP 源、GitHub 多包索引源、单仓库直接依赖的 config 语法、不可变提交解析与拒绝规则；**第一批联网** | 待开工（交接文档已就绪；**前置：ureq 是本仓首个网络依赖**） |
| 11A | [虚拟环境与包管理](11a-environments-and-packages.md) | 项目隔离、多源包仓库、联邦源索引、共享缓存、锁定和管理命令 | 进行中（D1、E0、E1、E2A、E2B、E3A 已实现；**E3B 的前置并发冻结件 11A-CONC 已就绪**；E3B 已实现；E3C 交接文档已就绪，E3D 与真实远程源尚未开始） |
