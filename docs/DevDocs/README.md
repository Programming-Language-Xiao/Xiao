...
| 11A-CONC | [并发模型：工具链内部并发与语言级并发的边界](11a-concurrency-model.md) | 划清工具链内部并发（E3B 前置，**冻结**）与语言级并发（归 `07-D`，**不冻结**）；冻结原语选型、有界并行、结果确定性、失败语义、跨进程锁 | 待冻结（设计冻结件；**§一 写进 `00-decisions` 后 E3B 才可依据**） |
