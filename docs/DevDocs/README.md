...
| 05 | [表、模块与工程模型](05-tables-and-projects.md) | 表生命周期、源码模块、依赖图和包外导出 | 进行中（05-A、05-B 已完成静态闭环） |
...
05 阶段当前完成了 05-A 和 05-B 的本地静态闭环：解析器能保留绝对导入 AST，模块分析器能从项目根发现 `.xiao` 文件、建立纯目录命名空间、解析词法绑定、传播顶层再导出并生成确定性依赖图。该闭环不读取 `config.xiao`、不下载外部依赖、不执行模块初始化，也不提供 Runtime 值；接手实现时先阅读 [05A. 导入语法](05a-import-syntax.md) 和 [05B. 本地模块解析](05b-local-module-resolution.md)，再进入尚未开始的表/配置/包边界任务。
