...
| 09R2 | [字节码寄存器机型特别研究](09r-bytecode-machine-research.md) | 三种机型的可运行原型、共享语义向量、事件接收器和容器/选择器原型路径 | 进行中（R2a 第一批：TAC 降低、对账验证器、栈式解释器、语义向量；第二批：容器对象、精确索引、临时值释放；R2C：异常控制流；R2D 待做） |
