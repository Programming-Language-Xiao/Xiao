...
| 11A-E3A | [包源契约与多源配置](11ae3a-package-source-contract.md) | 源描述与源身份、适配器接口、索引快照与联邦合并键、跨源选择与歧义诊断、RFC 8785 规范化；**契约批次，不联网** | 待开工（交接文档已就绪；**前置：冻结 7 条**，含 `11a:259` 的源字段名待冻结；写回与远程传输分属 E3D/E3B） |
| 11A | [虚拟环境与包管理](11a-environments-and-packages.md) | 项目隔离、多源包仓库、联邦源索引、共享缓存、锁定和管理命令 | 进行中（D1、E0、E1、E2A、E2B 已完成；E3A 交接文档已就绪，E3A–E3D 与远程源尚未开始） |
