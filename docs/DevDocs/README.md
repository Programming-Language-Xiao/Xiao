...
| 11A-E2B | [同步与安装命令](11ae2b-sync-and-install.md) | `sync` 的创建/补齐/激活流水线、`install`/`i` 的目标选择、激活通道与 11B 只读视图；**本批首次新增用户可见命令** | 待开工（交接文档已就绪；**前置：激活通道是全新机制**，`XIAO_ACTIVE_ENV` 现不可导出且只有名称） |
| 11A | [虚拟环境与包管理](11a-environments-and-packages.md) | 项目隔离、多源包仓库、联邦源索引、共享缓存、锁定和管理命令 | 进行中（D1、E0、E1、E2A 已完成；E2B 的命令接线与 E3 的远程源尚未开始） |
