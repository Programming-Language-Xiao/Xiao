...
- 禁止循环依赖、双向调用和通过私有字段建立隐式依赖。
- 模块之间只能通过明确、可测试、可版本化的数据结构或 trait 通信；不得复制另一层的语义实现。
- 语法层不得依赖类型系统、Runtime、VM、LLVM、CLI 或平台层；类型层只消费公开 AST、源码位置和诊断接口。
- CLI 不得重新实现解析、类型检查、优化或归档逻辑；平台适配不得改变语言语义。
- 新功能必须优先放入职责单一的子模块，禁止继续向中心文件堆叠实现。
- 每个模块必须在 README 中声明输入、输出、允许依赖和禁止依赖；架构检查发现违规时阻止合并。
...
- 单个实现文件不得成为跨层共享状态的中心；需要共享时提取最小稳定接口或独立数据类型。
