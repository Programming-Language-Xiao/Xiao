...
冻结 P1 的运算、调用、索引、代码块、类型声明或容器语义。P1 快照另位于
[`tests/spec/03-expression`](../03-expression/README.md)。
...
正例和一份负例，并在 DevDocs 与 UseDocs 中说明边界。P1 的正反例遵循同一约定，
但只断言表达式/选择器 AST 可构造及稳定诊断，不断言执行结果。
